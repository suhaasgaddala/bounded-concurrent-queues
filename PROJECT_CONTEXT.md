...
|   |-- line64_global_index_spmc_concept.png
|   `-- line64_mpmc_selected_topologies.png
|-- benchmarks/
|   |-- CMakeLists.txt
|   |-- benchmark_support.h
|   |-- external_baselines/
...
PR #8 added optional external benchmark adapters for Boost.Lockfree,
moodycamel, rigtorp, and atomic_queue. Boost comparison works locally when
Boost 1.90.0_1 is installed. The valid Boost-inclusive comparison used capacity
`32768` because Boost.Lockfree rejected `65536` with a freelist limit.
...
README performance tests and benchmarks now use two local image assets:
`assets/line64_global_index_spmc_concept.png` and
`assets/line64_mpmc_selected_topologies.png`. The selected MPMC performance test
results show Line64 `MPMCQueue` ranking first in the local Apple M4 `1P/1C` and
`2P/2C` capacity-`32768` scenarios against the measured baselines shown in the
README. SPSC remained competitive with measured SPSC baselines but is not
presented as a visual win. SPMC remains reported separately because multicast
observations are not directly comparable to exclusive-pop throughput. Benchmark
JSONL output is generated by the executable so charts and tables can be
regenerated from measured data instead of hand-written samples.
